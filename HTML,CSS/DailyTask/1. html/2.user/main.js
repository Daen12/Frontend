...
/////////////////////////////////////////
//  function만 만들어놓고 html파일에서 onclick 프로퍼티 설정함.
/////////////////////////////////////////
...
function regist() {
  ...
  //or 이므로 하나라도 false이면 => true가 되어 실행됨.
...
function login() {
  let id = document.getElementById("id").value;
  let password = document.getElementById("password").value;
  //user 정보를 자바스크립트 형태로 반환
  const user = JSON.parse(localStorage.getItem("user"));
  //만약에 id와 password가 존재하고,(null이 아니고) 등록된 정보와 같다면 (모두 &&로 묶여서 하나라도 틀리면 함수시행안됨)
  if (
    user.id &&
    user.password &&
    user.id === id &&
